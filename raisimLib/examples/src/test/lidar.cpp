...
int main(int argc, char* argv[]) {
  ...
  auto hm = world.addGround(0.0, "glass");//world.addHeightMap(0.0, 0.0, terrainProperties);
  ...
  auto scans = server.addInstancedVisuals("scan points",
                                          ...
                                          {1,0,0,1},
                                          {0,1,0,1});
  int scanSize1 = 1; //forward
  int scanSize2 = 50; //wide
  ...
  for(int time=0; time<1000000; time++) {
    ...
    for(int i=0; i<scanSize1; i++) {
      for (int j = 0; j < scanSize2; j++) {
        const double yaw = j * M_PI / scanSize2 * 0.6 - 0.3 * M_PI; //jth angle(-0.3*M_PI : for align yaw axis)
        double pitch = 0;//-(i * 0.3/scanSize1) + 0.2;
        ...
        if (col.size() > 0) {
          ...
          float length = (col[0].getPosition() - lidarPos.e()).norm(); // length of lidar's lazer
          scans->setColorWeight(i * scanSize2 + j, std::min(length/15.f, 1.0f)); //point color
        }
        else
